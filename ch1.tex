...
% this section is based on translation of https://e-x-a.org/rp/
...
The main method of interpreting your output is by rigorously measuring its properties (see above) and showing that the measurements fit your goals. The measurements should provide undisputable supporting evidence to allow you claim the goals (and partial goals) that were (and were not) fulfilled. Ideally, you will also compare your results with ones from people who solved similar problems --- besides measuring your own solution, you might also need to measure the alternative implementations and approaches for this purpose, use a comparable measurement methodology (benchmark) to allow comparison to other benchmarked work, and ensure fair comparison conditions. \emph{However, no one expects you to win this comparison! The thesis is not a competition!} A negative result is also a result, and thesis reviewers will understand it; especially if the apparent failure is well-supported by measurements and used as a groundwork for some constructive recommendation to prevent this kind of failure for others. Conversely, reviewers will show no understanding for irreproducible measurements, unfair comparisons, and hiding the negative results.
\item[How am I going to fill all these pages with text?]
...
The additional questions below do not directly relate to topic choice, but you may still want to equip yourself with suitable answers rather early, as they naturally shape the execution of the whole thesis:

\begin{description}
\item[Why do I have to explain everything in text again?]
You may wonder why you need to ``wrap'' your existing results (programs, measurements, algorithms, methods) in additional paper, despite of the fact that they are already visibly correct and solve the problem. First, the human-readable thesis serves as a singular artifact ensuring that your results will be understood and interpretable through \emph{centuries}!\footnote{University archives allow you to read and understand a thesis from 18\textsuperscript{th} century.} This starkly contrasts with other short-lived artifacts: Measurements will come out completely different on hardware that appears in 5 years from now, and computer code (as well as laboratory protocols) will typically cease to work in a decade because the ecosystem will develop. Second, the thesis serves as a comprehension check: Indirectly, to be able to teach your approach to your peers reliably, you need to understand it very well. Make your thesis such that it conveys a piece of your own knowledge further, towards future scholars.
\item[Am I finishing it in time?]
Allocate a lot of time for writing, editing, and making good pictures. At the \emph{extreme barest minimum}, allocate at least one month of work for fixing and incorporating the comments from your supervisor, especially if you never wrote a thesis before. Do not expect supervisors to read your input immediately, they generally do not have time for that --- instead, reserve at least a week for each revision. Check the thesis submission requirements in advance. In particular, if you are supposed to print your thesis on paper, order the printing service with sufficient time reserve.
...
First chapter usually builds the theoretical background necessary for readers to understand the rest of the thesis, including your own contribution. You should summarize and reference a lot of existing literature and research.

You should use the standard \emph{citations}\todo{Use \ttfamily{\textbackslash emph} command like this, to highlight the first occurrence of an important word or term. Reader will notice it, and hopefully remember the importance.}.

\begin{description}
\item[Obtaining bibTeX citation] Go to Google Scholar\footnote{\url{https://scholar.google.com}}\todo{This footnote is an acceptable way to `cite' web pages or URLs. Documents without proper titles, authors and publishers generally do not form citations. For this reason, avoid citations of Wikipedia pages.}, find the relevant literature, click the tiny double-quote button below the link, and copy the bibTeX entry.
